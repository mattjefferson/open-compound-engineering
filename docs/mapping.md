...
Source: `compound-engineering` plugin v2.35.1 from [EveryInc/compound-engineering-plugin](https://github.com/EveryInc/compound-engineering-plugin)
Install via: `scripts/install.sh --target <claude|codex|gemini>`
...
| `heal-skill.md` | `commands/heal-skill.md` | active | Fix incorrect SKILL.md files with wrong instructions or outdated API references |
| `lfg.md` | `commands/lfg.md` | active | Full autonomous engineering workflow |
| `resolve_parallel.md` | `commands/resolve_parallel.md` | active | Resolve all TODO comments using parallel processing |
...
| `every-style-editor` | — | **removed** | Edit text to conform to editorial style guide |
...
| `rclone` | — | **removed** | Upload, sync, and manage files across cloud storage |
| `resolve-pr-parallel` | `resolve_pr_parallel` | **merged** | Resolve PR comments in parallel (see "Generated from commands") |
...
| `figma-design-sync` | `agents/design/figma-design-sync` | — | **removed** | Detects and fixes visual differences between web and Figma |
...
| `schema-drift-detector` | `agents/review/schema-drift-detector` | — | **removed** | Detects unrelated schema.rb changes in PRs |
...
| `triage` | active | Triage and categorize findings for todo tracking (from upstream command) |
| `xcode-test` | active | iOS simulator testing (ported from agent-scripts archive) |
...
| `heal-skill` | `commands/heal-skill.md` | active | Backs the heal-skill command |
| `lfg` | `commands/lfg.md` | active | Backs the lfg command |
| `resolve_parallel` | `commands/resolve_parallel.md` | active | Backs the resolve_parallel command |
...
|---|---|---|---|---|---|
| Commands | 22 (17 + 5 workflows) | 12 | — | — | `commands/` |
| Skills (direct copy) | 20 | 14 | 5 | 0 | `skills/` |
| Skills (from agents) | 31 | 19 | 12 | — | `skills/` |
| Skills (from commands) | 13 | 13 | — | — | `skills/` |
| Skills (new) | 3 | 3 | — | — | `skills/` |
| **Total skills** | **61** | **49** | **17** | **0** | `skills/` |

## Change Log

| Date | Change | Details |
|---|---|---|
| 2026-02-20 | Mapping doc refresh | Updated command list and skill totals to reflect repository contents (including `heal-skill`, `lfg`, `resolve_parallel`) |
| 2026-02-19 | Ref / Context7 doc tool wording | Replaced `mcp__context7__` MCP call syntax and Context7-only references with generic "Ref / Context7" doc tool phrasing in `verify-skill` and `create-domain-expertise-skill` workflows |
| 2026-02-19 | Extracted `lfg` skill | Moved `lfg` command logic into `skills/lfg/SKILL.md`, command now delegates to skill |
| 2026-02-19 | Upstream sync v2.35.1 | Added System-Wide Test Check to `workflows-work` task loop, added System-Wide Impact sections to `plan-standard` and `plan-comprehensive` templates, added mandatory Write Plan File step to `workflows-plan` |
| 2026-02-19 | Added deepen-plan to plan menu | Added `/deepen-plan` as option in `workflows-plan` post-planning menu, cleaned up `spec-flow-analyzer` reference |
| 2026-02-19 | Codex workflow compatibility | Updated workflow and plan deepening skills to avoid Claude-only tool assumptions, use zsh-safe file discovery, and document runtime directory creation in target repos |
| 2026-02-19 | Clarified swarm mode | Marked `workflows-work` Swarm Mode as Claude Code only and added Codex approximation guidance |
...
| 2026-02-18 | Created `xcode-test` | Ported from agent-scripts archive (was `test-xcode`) |
| 2026-02-18 | Normalized skill references | Replaced `/skill-name` and `/workflows:skill` with consistent `skill-name` |
| 2026-02-18 | Renamed `EVERY_WRITE_STYLE.md` → `style.md` | Made style-editor reference file agent-agnostic |
| 2026-02-18 | Added `scripts/install.sh` | Multi-target installer for claude, codex, gemini |
| 2026-02-18 | Removed `config.toml` | MCP server config no longer shipped |
| 2026-02-18 | Fixed `resolve_pr_parallel` paths | Relative paths instead of `${CLAUDE_PLUGIN_ROOT}`, fixed underscore mismatch |
| 2026-02-18 | Removed `style-editor` | Not general purpose, cleaned cross-references |
| 2026-02-18 | Removed `rclone` | Not general purpose, cleaned cross-references |
| 2026-02-18 | Removed `schema-drift-detector` | Ruby/Rails-specific (schema.rb), cleaned cross-references |
| 2026-02-18 | Created `triage` skill | Ported from upstream command, made agent-agnostic |
| 2026-02-18 | Replaced `TodoWrite` refs | Generic task terminology across skills |
| 2026-02-18 | Removed rclone from `feature-video` | Removed upload steps and R2-specific URLs |
| 2026-02-18 | Removed `figma-design-sync` | Standalone skill, not part of base workflow |
| 2026-02-18 | Fixed skill path references | Replaced `${CLAUDE_PLUGIN_ROOT}` in git-worktree, `.agents/skills/` in workflows-review and best-practices-researcher with correct relative/discovery paths |
| 2026-02-18 | Fixed `workflows-review` formatting | Renumbered sections (1→6), fixed broken code fence around summary template, fixed truncated sentence, removed stray fences |
| 2026-02-18 | Fixed `spec-flow-analyzer` formatting | Promoted "Key principles" to proper heading |
| 2026-02-18 | Normalized `sub-agent` terminology | Replaced `sub-agents`/`sub-agent` with `subagents`/`subagent` in workflows-review |
